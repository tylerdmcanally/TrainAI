...
export default function ClientDashboardPage() {
  const { isAuthorized, isOwner, isEmployee, loading } = useAuthGuard()
  ...
  useEffect(() => {
    if (!isAuthorized || !user || !profile) return
    ...
    const fetchData = async () => {
      ...
      try {
        if (isOwner) {
          ...
          setAssignments(data || [])
          setTrainings(data?.map(a => a.training_modules).filter(Boolean) || [])
        }
      } catch (error) {
    ...
    fetchData()
  }, [isAuthorized, user, profile, isOwner])
